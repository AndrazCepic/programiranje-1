...
cat_directory = 'cats_dir'
# ime datoteke v katero bomo shranili glavno stran
frontpage_filename = 'cats.html'
# ime CSV datoteke v katero bomo shranili podatke
csv_filename = 'cats_data.csv'
...
def get_dict_from_ad_block(block):
    ...
    """
    regex = r'title="(?P<naslov>.*?)".*</h3>\s*(?P<opis>.*?)\s*<div.*<b>(?P<rodovnik>.*?)</b>.*<div class="price">(?P<cena>.*?)</div>'
    return re.search(regex, block, re.DOTALL).groupdict()
...
def ads_from_file(filename, directory):
    ...
    return [get_dict_from_ad_block(block) for block in ads if get_dict_from_ad_block(block)]
